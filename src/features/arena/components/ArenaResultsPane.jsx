...
const exportResultsToPdf = ({ title, filteredResults, analytics, t }) => {
  const rows = filteredResults
    .map(
      (result) => `
        <tr>
          <td>${escapeHtml(result.participantName || t("arenaShared.results.unknownUser"))}</td>
          <td>${escapeHtml(result.groupName || t("arenaShared.results.noGroup"))}</td>
  ...
  const html = `
    ...
    <html lang="uz">
      <head>
        <meta charset="utf-8" />
        <title>${escapeHtml(title)} - ${escapeHtml(t("arenaShared.results.reportTitle"))}</title>
        <style>
      ...
      <body>
        <div class="head">
          <h1>${escapeHtml(title)} ${escapeHtml(t("arenaShared.results.resultsSuffix"))}</h1>
          <p class="sub">${escapeHtml(t("arenaShared.results.filteredReport"))}</p>
        </div>
        <section class="stats">
          <div class="card"><div class="label">${escapeHtml(t("arenaShared.results.submitted"))}</div><div class="value">${analytics.submittedCount}</div></div>
          <div class="card"><div class="label">${escapeHtml(t("arenaShared.results.mastery"))}</div><div class="value">${analytics.mastery}%</div></div>
          <div class="card"><div class="label">${escapeHtml(t("arenaShared.results.easiest"))}</div><div class="meta">${escapeHtml(analytics.easiest?.prompt || t("arenaShared.results.noData"))}</div><div class="value">${analytics.easiest?.rate ?? 0}%</div></div>
          <div class="card"><div class="label">${escapeHtml(t("arenaShared.results.hardest"))}</div><div class="meta">${escapeHtml(analytics.hardest?.prompt || t("arenaShared.results.noData"))}</div><div class="value">${analytics.hardest?.rate ?? 0}%</div></div>
        </section>
        <table>
          <thead>
            <tr>
              <th>${escapeHtml(t("arenaShared.results.student"))}</th>
              <th>${escapeHtml(t("arenaShared.results.group"))}</th>
              <th>${escapeHtml(t("arenaShared.results.date"))}</th>
              <th>${escapeHtml(t("arenaShared.results.correct"))}</th>
              <th>${escapeHtml(t("arenaShared.results.total"))}</th>
              <th>${escapeHtml(t("arenaShared.results.percent"))}</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
    ...
    <Overlay onClick={onClose}>
      <Pane onClick={(event) => event.stopPropagation()}>
        <Header>
          ...
          <Controls>
            <SearchWrap>
              ...
              <SearchInput
                placeholder={
                  searchPlaceholder || t("arenaShared.results.searchPlaceholder")
                }
                value={searchTerm}
            ...
            >
              <option value="all">{t("arenaShared.results.allGroups")}</option>
        ...
        <Content>
          <DashboardGrid>
            <DashboardCard>
              <DashboardLabel>{t("arenaShared.results.submitted")}</DashboardLabel>
              <DashboardValue>{analytics.submittedCount}</DashboardValue>
              <DashboardMeta>{t("arenaShared.results.filteredCount")}</DashboardMeta>
            </DashboardCard>
            <DashboardCard>
              <DashboardLabel>{t("arenaShared.results.mastery")}</DashboardLabel>
              <DashboardValue>{analytics.mastery}%</DashboardValue>
              <DashboardMeta>{t("arenaShared.results.average")}</DashboardMeta>
            </DashboardCard>
            <DashboardCard>
              <DashboardLabel>{t("arenaShared.results.easiest")}</DashboardLabel>
              <DashboardValue>{analytics.easiest?.rate ?? 0}%</DashboardValue>
              <DashboardMeta>
                {analytics.easiest?.prompt || t("arenaShared.results.noData")}
              </DashboardMeta>
            </DashboardCard>
            <DashboardCard>
              <DashboardLabel>{t("arenaShared.results.hardest")}</DashboardLabel>
              <DashboardValue>{analytics.hardest?.rate ?? 0}%</DashboardValue>
              <DashboardMeta>
                {analytics.hardest?.prompt || t("arenaShared.results.noData")}
              </DashboardMeta>
            </DashboardCard>
          </DashboardGrid>

          <SectionTitle>{t("arenaShared.results.allResults")}</SectionTitle>
          ...
          {loading ? (
            ...
            <EmptyState>{t("arenaShared.results.empty")}</EmptyState>
          ) : (
            <TableWrap>
              <TableScroller>
                <Table>
                  <thead>
                    <tr>
                      <Th>{t("arenaShared.results.student")}</Th>
                      <Th>{t("arenaShared.results.group")}</Th>
                      <Th>{t("arenaShared.results.date")}</Th>
                      <Th>{t("arenaShared.results.correct")}</Th>
                      <Th>{t("arenaShared.results.total")}</Th>
                      <Th>{t("arenaShared.results.percent")}</Th>
                      <Th />
                    </tr>
                  </thead>
                  <tbody>
                          ...
                          >
                            <Td>
                              <NameCell>
                                ...
                                <Subtext>
                                  {t("arenaShared.results.questionCount", {
                                    count: (result.breakdowns || []).length,
                                  })}
                                </Subtext>
                              </NameCell>
                            </Td>
                                    ...
                                    >
                                      <BreakdownTop>
                                        <BreakdownPrompt>
                                          {index + 1}.{" "}
                                          {entry.prompt ||
                                            t("arenaShared.results.question")}
                                        </BreakdownPrompt>
                                        <StatusBadge $correct={entry.isCorrect}>
                                          {entry.isCorrect
                                            ? t("arenaShared.results.correct")
                                            : t("arenaShared.results.wrong")}
                                        </StatusBadge>
                                      </BreakdownTop>
                                      ...
                                      {Array.isArray(entry.selectedTokens) ? (
                                        <>
                                          <BreakdownText>
                                            {t("arenaShared.results.yourSentence")}
                                          </BreakdownText>
                                          <TokenLine>
                                            {(entry.selectedTokens || []).length ? (
                                              ...
                                              <BreakdownText>
                                                <strong>
                                                  {t("arenaShared.results.noAnswer")}
                                                </strong>
                                              </BreakdownText>
                                            )}
                                          </TokenLine>
                                          ...
                                          <BreakdownText>
                                            {t("arenaShared.results.correctAnswer")}
                                          </BreakdownText>
                                          <TokenLine>
                                            ...
                                            <MistakeList>
                                              {(entry.mistakes || []).map(
                                                (mistake, mistakeIndex) => (
                                                  ...
                                                  >
                                                    {t(
                                                      "arenaShared.results.mistakeTemplate",
                                                      {
                                                        position:
                                                          mistake.position,
                                                        actual:
                                                          mistake.actual ||
                                                          t(
                                                            "arenaShared.results.nothing",
                                                          ),
                                                        expected:
                                                          mistake.expected ||
                                                          t(
                                                            "arenaShared.results.extraToken",
                                                          ),
                                                      },
                                                    )}
                                                  </MistakeItem>
                                        ...
                                        <>
                                          <BreakdownText>
                                            {t("arenaShared.results.selectedAnswer")}
                                            :{" "}
                                            <strong>
                                              {entry.selectedText ||
                                                t("arenaShared.results.noAnswer")}
